...
bool UiVuGradient::Configure(UiPalette &palette, std::uint16_t height) {
  if (height == 0 || height > kMaximumHeight)
    return false;
  ...
  if (palette.VuGradientCurrent(height))
    return true;
